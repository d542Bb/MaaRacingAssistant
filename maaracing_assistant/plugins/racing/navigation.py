...
class Navigation:
    ...
    def __init__(self, proj: Path, debug, ctx: ActivityContext):
        self.proj = proj
        self.debug = debug
        # 集中解构 capability（一次性 narrowing，避免模块内到处 assert）
        self._lifecycle = ctx.lifecycle
        self._capture = ctx.capture
        self._gamepad = ctx.gamepad
        if self._capture is None or self._gamepad is None:
            raise RuntimeError("Navigation 需要 capture + gamepad 能力")
    ...
    # ── 能力代理（不再反引 controller） ──

    @property
    def _running(self) -> bool:
        return self._lifecycle.running

    def _gpad_lease(self) -> AbstractContextManager[Gamepad]:
        """打开一个手柄租约：进入复用/获取手柄，退出归零归还。"""
        return self._gamepad.acquire()
    ...
    def _interruptible_sleep(self, seconds: float):
        self._lifecycle.sleep(seconds)

    def _stop_stick(self, gpad: Gamepad):
        gpad.left_joystick(x_value=0, y_value=0)
        gpad.update()
        self._last_stick = (0, 0)

    def _press_button(self, gpad: Gamepad, button, duration: float = 0.3):
    ...
    def _move_cursor_to_target(self, cursor_pos: tuple, target_pos: tuple,
                                gpad: Gamepad, stop_distance: int = 25,
    ...
    def _ensure_cursor(self, gpad: Gamepad):
        """如果截图找不到光标，4方向推摇杆搜索"""
        arr = self._screencap()
        if arr is not None:
    ...
    def _blind_move(self, gpad: Gamepad, last_pos: tuple, target: tuple, elapsed: float):
        """光标丢失后盲操推摇杆"""
    ...
    def _press_and_verify(self, gpad: Gamepad, cursor_area: float,
