...
>
> - 鉴宝域：[../treasure/CODE\_WIKI.md](../treasure/CODE_WIKI.md)
...
4. [决策算法详解](#4-决策算法详解)
5. [赛车控制参数](#5-赛车控制参数)
6. [RacingLoop.\_run\_impl 核心循环](#6-racingloop_run_impl-核心循环)
7. [赛车控制坑点](#7-赛车控制坑点)
8. [Navigation 光标导航引擎](#8-navigation-光标导航引擎)
9. [模型与插件资源（自包含）](#9-模型与插件资源自包含)

***

## 1. RacingLoop 赛车自动驾驶循环

[racing\_loop.py](file:///d:/maaracing_assistant/maaracing_assistant/plugins/racing/loop.py)
...
- 前馈瞄准（动态stop\_zone+目标移动预测+预见性提前收敛）

- moving\_to\_center 单一衰减（近区回摆×mv 合并禁止双重相乘）
...
- YOLO\_INTERVAL = 2：每2帧推理1次

- SLOW\_CHECK = 15：每秒（\~15fps）检查一次结束画面
...
| 方法/属性                                          | 说明                                                             |
| ---------------------------------------------- | -------------------------------------------------------------- |
| `__init__(model_path, debug, gpad=None)`       | 初始化YOLO检测器、加载结束模板；gpad 经 bind\_gpad 注入快捷键                      |
...
| `stop()`                                       | 停止运行、重置所有状态（手柄归零由外层 module 的 reset\_device 处理）                 |
...
| `_zone_boundaries`                             | 属性：返回(horizon, far\_bot, mid\_bot, roi\_bot)                   |
...
[racing\_module.py](file:///d:/maaracing_assistant/maaracing_assistant/plugins/racing/module.py)
...
| `stop()`             | 幂等停止：先停 racing\_loop，再中断 MAA Pipeline（`tasker.post_stop()`）                   |
| `cleanup()`          | 幂等释放模块资源（renderer 由 Context.close 释放）                                         |
| `current_stage`      | 属性：当前阶段名                                                                      |
| `_ensure_pipeline()` | 首次比赛前创建 Tasker/Resource/RacingLoop 并 post\_bundle + bind + add\_context\_sink |
...
[racing\_renderer.py](file:///d:/maaracing_assistant/maaracing_assistant/plugins/racing/renderer.py)
...
| 中区力度系数                       | 1.0                              | 中区基础系数                                   |
| 近区力度系数                       | 0.6                              | 近区防撞降低系数                                 |
| off\_center远区保底              | 15%（v0.11.1）                     | 目标不在中线车道时最低力度                            |
| off\_center中区保底              | 25%（v0.11.1）                     | <br />                                   |
| off\_center近区保底              | 40%（v0.11.1）                     | <br />                                   |
| stop\_zone公式                 | 0.01 + min(0.10, area\_ratio×30) | 仅 in\_center=True 生效                     |
| off\_center死区effective\_stop | 0.01（v0.11.1）                    | 偏离中线车道收紧死区，防止吞偏移                         |
| C区突发帧数                       | 2帧                               | 短促打满改变指向                                 |
| C区滑行帧数                       | 5帧                               | 归中滑行远离墙                                  |
...
| 分位统计                     | 同上                    | P50 / P90\_trim / P95\_raw              | 稳健统计组合                        |
| FPS 公式                   | 同上                    | `fps = round(950 / yolo_p90_trim)`      | 5% 裕量（预算 105% P90\_trim）      |
...
| 标线不推断对侧                     | \_detect\_lane只返回真实检出的side+pos，调用方必须用.get()安全访问 |
| 防撞记忆wall\_memory            | 标线存在时记录靠墙状态，标线丢失+无YOLO目标时轻柔回带                   |
| \_aim\_at/\_avoid无标线边界约束    | 防碰撞由独立模块负责，金币/避障可自由变道                           |
| NMS per-class索引映射           | NMS返回类别内下标，需经cls\_local→mask\_indices两级映射回原数组   |
| \_avoid框重叠检测                | `left<R2c and right>L2c`，不用中心点，用车框左右沿判断         |
| 避障穿透                        | \_avoid返回0时\_decide不返回避障，落到金币/跳板车逻辑             |
| 车道保持方向统一                    | 左右标线侧`new_dir=1 if diff>0 else -1`，右标线侧不能取反     |
| 地平线推断                       | 低置信度(≤0.25)小面积(<400px²)小车，前40帧不推断，≥3车取1/4分位锁死   |
| \_lane\_boundaries\_at\_y透视 | 消失点(cx,horizon)经测量点线性外推，L2c/R2c在(0.22,1.00)测量   |
| 标线单边选择                      | side\_score=总长度×角度一致性，择优选一侧，只返回{side, pos}      |
...
- 经 `ctx` 能力门面访问截图/手柄（不再反向引用 controller 的私有 `_screencap`/`_gpad`）

**核心类**：`ButtonDef` / `Navigation`（构造签名 `__init__(proj: Path, debug, ctx: ActivityContext)`）
...
**预定义按钮**（在 RacingModule 中）：

- `BTN_极速狂飙入口`: (0.880, 0.720), activity\_page\_template, True, 50px

- `BTN_开始挑战`: (0.855, 0.898), activity\_page\_template, False, 12px

- `BTN_寻找对手`: (0.804, 0.753), find\_opponent\_template, False, 25px
...
- **独立死区**：非零轴值<4260（MAX\_AXIS的53%）时自动抬升到死区阈值

- 刹车时间：近距80ms / 远距50ms

- stop\_distance 自适应：`max(8, close_th × 0.55)`
...
| 方法                                             | 说明                                  |
| ---------------------------------------------- | ----------------------------------- |
| `__init__(proj, debug, ctx)`                   | 初始化，持有项目路径、调试器与能力门面 `ctx`           |
...
| 常量             | 值                      | 说明                         |
| -------------- | ---------------------- | -------------------------- |
| DEADZONE       | 4260                   | 摇杆死区阈值（MAX\_AXIS=8000的53%） |
| MAX\_AXIS      | 8000                   | 导航摇杆最大值（非赛车的32767）         |
...
| 微调脉冲时间         | 25ms                   | <35px时短推                   |
| 微调刹车时间         | 80ms                   | 微调后刹车                      |
| 收缩保底公式         | max(5, close\_th×0.65) | 按A失败后阈值收缩                  |
| stop\_distance | max(8, close\_th×0.55) | 自适应停止距离                    |
| 盲操超时           | 2秒                     | 光标丢失盲操超时                   |
...
模板清单（`plugins/racing/resources/image/`），命名格式 `{用途}_template.{ext}`，按钮定义和阈值在 ButtonDef 中同步维护：

| 文件                           | 尺寸        | 用途                                                      | 匹配阈值 | 状态       |
| ---------------------------- | --------- | ------------------------------------------------------- | ---- | -------- |
| `settings_page_template.jpg` | \~484×300 | 归位：识别设置页面（左上角 ROI 50%，彩色多尺度）                            | 0.65 | ✅ 正常     |
| `activity_page_template.jpg` | 1100×550  | 导航：识别活动页面 / 检测页面已离开（导航二"开始挑战"消失验证正反逻辑）                  | 0.70 | ✅ 正常     |
| `find_opponent_template.jpg` | 374×195   | 导航三：识别"寻找对手"页面，按钮消失=成功验证（`template_should_match=False`） | 0.55 | ✅ v0.5.0 |
| `store_popup_template.jpg`   | 159×262   | 商店弹窗检测 + `RacingLoop._is_end()` 结束画面检测（任一模板命中即返回 True）  | 0.55 | ✅ v0.6.0 |
| `round1_end_template.jpg`    | —         | 回合 1 结束画面检测；`_is_end` 模板列表中的主模板                         | 0.55 | ✅ v0.6.0 |
| `cursor_template.jpg`        | 168×176   | 旧光标模板匹配 → 已废弃，现改为"双中心面积评分"几何形状识别                        | —    | ❌ 已废弃    |
| `button_main_template.jpg`   | \~242×67  | 旧按钮位置模板 → 已废弃，现改为 ButtonDef.pct 百分比硬编码 + 页面模板验证         | —    | ❌ 已废弃    |

***

## 9. 模型与插件资源（自包含）

**插件自包含契约**：代码、模板、pipeline、YOLO 模型全部位于 `plugins/racing/` 内，registry 按 `manifest.py` 自动发现；整个目录拷走即卸载、放入即安装。

```
plugins/racing/
├── manifest.py                     # ID + MODULE_CLASS（registry 扫描入口）
├── __init__.py                     # PLUGIN_DIR / RES_DIR / MODEL_PATH 常量（资源统一入口）
├── resources/
│   ├── image/                      # 导航模板（§8 清单）
│   ├── pipeline/tasks.json         # MAA Pipeline 任务定义
│   └── onnx/
│       ├── model.onnx              # YOLO 模型（3类：coin/car/bonus_car，~10MB）
│       └── README.md               # 模型权重 AGPL-3.0 许可声明（Ultralytics 衍生）
```

- **模型路径**：`__init__.py` 的 `MODEL_PATH = RES_DIR / "onnx" / "model.onnx"`；目录名用 `onnx/` 避开 MAA Resource 保留的 `model/` 语义（post\_bundle 会扫描框架自有模型目录）

- **缺失校验**：模块类声明 `REQUIRED_ASSETS = ("resources/onnx/model.onnx",)`，sidecar 启动前逐项检查，缺失拦截并提示插件内路径；REQUIRES 不再含全局 "onnx" 能力

- **训练与导出**：`python tools/training/train.py`（训练 + 自动导出 ONNX + 复制到本插件 `resources/onnx/`）；数据集 3 类（coin/car/bonus\_car）见 `tools/training/dataset.yaml`；训练依赖（`ultralytics`/`torch`）为可选 extra `pip install -e .[train]`，不随运行时发布。导出坑点：`simplify=True` 可能产生损坏模型（主文档 §10.1）

- **发布**：模型随 `maaracing_assistant/` 整包 robocopy 进发布包（assemble.ps1 不再单列 assets\model）
