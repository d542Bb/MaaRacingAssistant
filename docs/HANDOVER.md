# MaaRacingAssistant v0.10.0 — 开发交接文档
...
- ✅ **黄色标线车道检测** — HSV 滤波（H:20-30, S:80-255, V:80-255）HoughLinesP 直线检测 ✅ v0.7.1
...
- ✅ **转向平滑校准** — alpha 校准状态机（baseline→steer→settle）嵌入主循环，dd 加速度检测 + 位移验证 + 失败重试 ✅ v0.10.0
- ✅ **C 区 cum3 位移过滤** — 3 帧累计位移 >10px 才触发 C 区，防止车道 1 正常行驶误触 ✅ v0.10.0
- ✅ **HSV 阴影标线检测** — S/V 下限从 150 降至 80，可识别阴影下的黄色标线 ✅ v0.10.0
- ✅ **Debug 实时值追踪** — `_apply_trigger` / `_steer` 封装手柄操作，debug 显示真实油门和摇杆值 ✅ v0.10.0
