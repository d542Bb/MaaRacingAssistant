...
## 决策优先级（RacingLoop._decide，贪婪模式）

```
1️⃣ 金币+奖励车 → 面积优先，面积近时选离X轴中线近的，前馈瞄准提前停止
2️⃣ 防碰撞 C 区（左墙 pos>450 且 cum3>10 / 右墙 pos<830 且 cum3<-10）→ 突发修正 2 帧 + 归中 5 帧
3️⃣ 障碍车（car）→ 框与中心区(L2c~R2c)重叠才躲，非重叠穿透到金币
...
- ✅ 转向平滑 — alpha=0.6 固定值（校准模块已删除，受干扰性太强）
- ✅ C 区 cum3 位移过滤 — 3 帧累计位移 >10px 才触发 C 区，防止车道 1 正常行驶误触
- ✅ HSV 阴影标线检测 — S/V 下限从 150 降至 80，可识别阴影下的黄色标线
- ✅ Debug 实时值追踪 — `_apply_trigger` / `_steer` 封装手柄操作，debug 显示真实油门和摇杆值
- ✅ 贪婪决策优先级 — 金币+奖励车优先（面积优先），防撞降级（撞车无惩罚）
- ✅ 前馈瞄准 — `_aim_at` 动态 stop_zone，根据目标大小预测提前停止转向
- ✅ 记录模式 — GUI 勾选后读取物理手柄，CSV 记录操作数据用于分析
- ✅ 车道保持变化率检测 — `_calc_drift` 复用，`abs(d)<5px` 提前停止修正
- ✅ 删除变道后激活车道保持 — 移除 `force_init`，只在无目标时激活
